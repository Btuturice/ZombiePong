...
namespace ZombiePong
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            {
                ball.Velocity = new Vector2(ball.Velocity.X * -1.05f, (float)Math.Sin(ball.Location.Y - paddle1.Center.Y) * -50);
            }
            if (ball.IsBoxColliding(paddle2.BoundingBoxRect) && ball.Location.Y != paddle2.Center.Y)
            {
                ball.Velocity = new Vector2(ball.Velocity.X * -1.05f, (float)Math.Sin(ball.Location.Y - paddle2.Center.Y) * -50);
            }
